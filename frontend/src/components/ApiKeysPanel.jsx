    ...
    <div className="flex-1 overflow-y-auto bg-slate-50">
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-semibold text-slate-900 mb-1">{T('API Access', 'API 访问')}</h3>
          ...
          {rawKey && (
            <div className="mt-3 p-3 rounded-lg bg-slate-900 text-slate-100 text-sm break-all">
              <div className="text-xs text-slate-400 mb-1">{T('Your API key (shown once):', '您的 API 密钥（仅显示一次）：')}</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100">
            <h3 className="font-semibold text-slate-900">{T('Your API keys', '您的 API 密钥')}</h3>
          </div>
          {loading ? (
            <div className="p-4 text-sm text-slate-500">{T('Loading…', '加载中…')}</div>
          ) : keys.length === 0 ? (
        ...
        {/* Docs */}
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-semibold text-slate-900 mb-2">{T('Quick start', '快速开始')}</h3>
